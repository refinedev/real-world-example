...
import { useGetIdentity, useTable, useDelete, useUpdate, useOne, useParse } from '@refinedev/core'
...
export const ProfilePage: React.FC = () => {
  ...
  const { tableQueryResult, current, pageCount, setCurrent, setFilters } = useTable<IArticle>({
    resource: 'articles',
    queryOptions: {
      ...
      <div className='container'>
        <div className='row'>
          <div className='col-xs-12 col-md-10 offset-md-1'>
            <ProfileNav params={params} profileData={profileData} setFilters={setFilters} />

            {tableQueryResult.isLoading && <div className='article-preview'>Loading articles...</div>}

            {!tableQueryResult.data?.total && <div className='article-preview'>No articles are here... yet.</div>}
            ...
            <Pagination total={pageCount} current={current} setCurrent={setCurrent} />
          </div>
        </div>
      </div>
    </div>
